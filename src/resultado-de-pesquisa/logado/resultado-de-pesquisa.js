...
const perfilUsuario = dadosDoUsuario.perfil; // "Pessoa Física" ou "Pessoa Jurídica"


const urlParams = new URLSearchParams(window.location.search);
const termo = urlParams.get("q")?.toLowerCase()?.normalize("NFD")?.replace(/[\u0300-\u036f]/g, "");
...
if (!termo || termo.trim() === "") {
  ...
  const resultados = conteudos
    ...
    .filter((conteudo) => {
      return (
        conteudo.perfil === perfilUsuario ||
        conteudo.perfil === "Ambos"
      );
    });

  if (resultados.length === 0) {
    resultadosDiv.innerHTML = `<p>Nenhum resultado encontrado para "<strong>${termo}</strong>" com perfil ${perfilUsuario}.</p>`;
  } else {
